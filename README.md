...
#### Сценарий 2: Работа персонала
1. Персонал заходит в бот для персонала в Telegram.
2. Персонал авторизуется, используя логин и пароль.
3. Персонал выбирает форму для отчета о выполненной работе.
4. Персонал заполняет форму, указывая номер комнаты и выполненные работы.
5. Персонал прикрепляет фотоотчет.
6. Персонал отправляет форму в хранилище через бота.
7. Персонал получает подтверждение об успешной отправке отчета.
